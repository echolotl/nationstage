...
export const filterMetadata: FilterMetadata[] = [
  { id: 'all', displayName: 'All Banners' },
  { id: 'agriculture', displayName: 'Agriculture' },
  { id: 'buildings', displayName: 'Buildings' },
  { id: 'cities', displayName: 'Cities' },
  { id: 'destruction', displayName: 'Destruction' },
  { id: 'economics', displayName: 'Economics' },
  { id: 'forests', displayName: 'Forests' },
  { id: 'grandeur', displayName: 'Grandeur' },
  { id: 'hotdry', displayName: 'Hot & Dry' },
  { id: 'industry', displayName: 'Industry' },
  { id: 'kulture', displayName: 'Culture' },
  { id: 'landscapes', displayName: 'Landscapes' },
  { id: 'military', displayName: 'Military' },
  { id: 'health', displayName: 'Health' },
  { id: 'oceans', displayName: 'Oceans' },
  { id: 'people', displayName: 'People' },
  { id: 'animals', displayName: 'Animals' },
  { id: 'ruins', displayName: 'Ruins' },
  { id: 'law', displayName: 'Law' },
  { id: 'towns', displayName: 'Towns' },
  { id: 'urban', displayName: 'Urban' },
  { id: 'violet', displayName: 'Violet' },
  { id: 'otherworldly', displayName: 'Otherworldly' },
  { id: 'x-factor', displayName: 'X-Factor' },
  { id: 'misc', displayName: 'Miscellaneous' }
];
...
export const idPrefixToFilter: Record<string, FilterType> = {
  'a': 'agriculture',
  'b': 'buildings',
  'c': 'cities',
  'd': 'destruction',
  ...
  'x': 'x-factor',
  'z': 'misc',
  'y': 'misc' // Additional banners
};
export type FilterType = 'all' | 'agriculture' | 'buildings' | 'cities' | 'destruction' | 
'economics' | 'forests' | 'grandeur' | 'hotdry' | 'industry' | 'kulture' | 
'landscapes' | 'military' | 'health' | 'oceans' | 'people' | 'animals' | 
'ruins' | 'law' | 'towns' | 'urban' | 'violet' | 'otherworldly' | 'x-factor' | 'misc';
// Add this helper function
export function getBannerFilter(bannerId: string): FilterType {
  const prefix = bannerId.charAt(0).toLowerCase();
  return idPrefixToFilter[prefix] || 'misc';
}

// Add this function to get all banners for a specific filter
export function getBannersByFilter(filter: FilterType): BannerData[] {
  if (filter === 'all') {
    return getAllBanners();
  }
  
  return Object.values(bannerDatabase).filter(banner => 
    getBannerFilter(banner.id) === filter
  );
}
...
export function getBannerById(id: string): BannerData | undefined {
  ...
  export function getBannerByTitle(title: string, exact: boolean = false): BannerData | undefined {
    const normalizedSearch = title.toLowerCase();
    
    return Object.values(bannerDatabase).find(banner => {
        const bannerTitle = banner.title.toLowerCase();
        return exact ? bannerTitle === normalizedSearch : bannerTitle.includes(normalizedSearch);
    });
}